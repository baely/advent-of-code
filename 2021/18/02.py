...
class SnailFish:
    ...
    def copy(self, new_parent: Optional["SnailFish"] = None) -> "SnailFish":
        new_snail_fish = SnailFish(new_parent, None, None)
        new_snail_fish.left = self.left if isinstance(self.left, int) else self.left.copy(new_snail_fish)
        new_snail_fish.right = self.right if isinstance(self.right, int) else self.right.copy(new_snail_fish)
...
for a in snail_fish:
    for b in snail_fish:
        if a is b:
            continue
        a1, b1 = a.copy(), b.copy()
        magnitudes.append((a1 + b1).get_magnitude())
