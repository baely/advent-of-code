...
if [ -d "$TARGET_DIR" ]; then
    ...
    # Generate file names for part 1 and part 2
    FILE_NAME_1="Solution${TARGET_DAY}01"
    FILE_NAME_2="Solution${TARGET_DAY}02"

    # Check if we should copy Solution*01.java to Solution*02.java
    if [ -f "$TARGET_DIR/$FILE_NAME_1.java" ] && { [ ! -f "$TARGET_DIR/$FILE_NAME_2.java" ] || [ ! -s "$TARGET_DIR/$FILE_NAME_2.java" ]; }; then
        cp "$TARGET_DIR/$FILE_NAME_1.java" "$TARGET_DIR/$FILE_NAME_2.java"
    fi
    # Open Solution*02.java in IntelliJ
    idea "$TARGET_DIR/$FILE_NAME_2.java"
else
    # First run - create directory and copy template
    mkdir -p "$TARGET_DIR"

    # Generate file name for part 1
    FILE_NAME_1="Solution${TARGET_DAY}01"

    # Copy template and replace placeholders
    cp ./template/Solution1.java "$TARGET_DIR/$FILE_NAME_1.java"
    sed -i '' "s|./YYYY/DD/input.txt|./$TARGET_YEAR/$TARGET_DAY/input.txt|g" "$TARGET_DIR/$FILE_NAME_1.java"

    cp ./template/input.txt "$TARGET_DIR/"

    # Open files in IntelliJ
    idea "$TARGET_DIR/input.txt"
    idea "$TARGET_DIR/$FILE_NAME_1.java"
fi